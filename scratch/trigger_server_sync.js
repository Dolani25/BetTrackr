...
async function runEndToEndTest() {
  ...
  try {
    ...
    if (!user) {
      console.error('❌ User not found with a Football.com account linked.');
    ...
    // Let's clear existing Football.com bets to ensure the sync fully works and fetches them all
    console.log('🗑️ Clearing existing Football.com bets for this user to test fresh population...');
    const deleteResult = await Bet.deleteMany({ userId: user.userId, bookie: 'Football.com' });
    ...
    // Wait 15 seconds for server to boot
    ...
    while (isSyncing && attempts < 100) {
      ...
      const currentBets = await Bet.countDocuments({ userId: user.userId, bookie: 'Football.com' });
    ...
    const finalBetsCount = await Bet.countDocuments({ userId: user.userId, bookie: 'Football.com' });
